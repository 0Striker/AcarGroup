...
public partial class CompanyInfoController
{
    ...
    public async Task<ActionResult> UploadHeroImage(IFormFile image)
    {
        try
        {
            ...
            var imageUrl = await _fileUploadService.UploadImageAsync(stream, image.FileName, "company");
    ...
    public async Task<ActionResult> UploadSliderImage(int slot, IFormFile image)
    {
        try
        {
            ...
            using var stream = image.OpenReadStream();
            var imageUrl = await _fileUploadService.UploadImageAsync(stream, image.FileName, "company/slider");
